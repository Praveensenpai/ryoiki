...
pub fn select_modules() -> Result<Option<Vec<String>>> {
    // If not running in an interactive terminal, default to all modules
    if !io::stdin().is_terminal() {
        let all = get_available_modules().into_iter().map(|m| m.id.to_string()).collect();
        return Ok(Some(all));
    }
    ...
    loop {
        terminal.draw(|f| {
            ...
            let title = Line::from(vec![
                Span::styled(" 領域 ", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
                Span::styled("Ryoiki ", Style::default().fg(Color::White).add_modifier(Modifier::BOLD)),
                Span::styled("— Minimal Ubuntu Server Setup", Style::default().fg(Color::DarkGray)),
            ]);
                ...
                match key.code {
                    KeyCode::Up | KeyCode::Char('k') => {
                        if *cursor > 0 {
                            ...
                            *cursor = modules.len() - 1;
                        }
                    }
                    KeyCode::Down | KeyCode::Char('j') => {
                        if *cursor + 1 < modules.len() {
                            *cursor += 1;
                        } else {
                            *cursor = 0;
                        }
